...
if __name__ == "__main__":
    ...
    #   trainsets       backbone是在哪个数据集上预训练的
    #----------------------------------------------------------------------------------------------------------------------------#
    pretrained      = True
    trainsets       = 'ImageNet'
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    Freeze_Train        = True if pretrained else False
    ...
    model = FasterRCNN(num_classes, anchor_scales = anchors_size, backbone = backbone, pretrained = pretrained, trainsets=trainsets)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    if True:
        ...
        #   记录eval的map曲线
        #----------------------#
        map_out_path = ".{}".format(trainsets)
        temp_map = eval_callback   = EvalCallback(model_train, input_shape, class_names, num_classes, val_lines, log_dir, Cuda, \
                                        map_out_path=map_out_path, eval_flag=eval_flag, period=eval_period)
        if temp_map:
            try:
                loss_history.wandb.log({"map": temp_map})
            except:
                pass
        #---------------------------------------#
        #   开始模型训练
